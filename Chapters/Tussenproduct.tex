...
In regel 4 en daaropvolgende regels worden de variabelen beschreven die de auto nodig heeft om zichzelf rechtuit te laten rijden met behulp van de BNO055\cite{AXIS}, zoals beschreven in regels 23 tot en met 30.

In het geval dat de ultrasone sensoren een foutieve waarde afgeven en de \gls{Smart-Car} niet in staat is om rechtdoor te rijden maar dit toch blijft doen en dus vast komt te zitten, zal dit worden gedetecteerd en zal de auto zichzelf uit deze situatie bevrijden, zoals beschreven in regels 54 en 55.
\begin{lstlisting}
    int sidecount = 0;
int rotation = 500;
int driven = 0;
...
\end{lstlisting}
Om de gemiddelde snelheid van een auto te berekenen, moeten we de vectoriële som van de acceleratiesnelheden uitvoeren.
\begin{equation}
